...
class PdoDataSet implements Iterator, Countable, ArrayAccess, ISqlSelectOptions, ISqlConditions {
	...
	function ilike($lhs, $rhs) { $copy = clone $this; $copy->sqlBuilder->ilike($lhs,$rhs); return $copy; }
	// XXX further like/ilike references
	/**
	 * @see SqlBuilder::like
	 * @return PdoDataSet
	 */
	function notLike($lhs, $rhs) { $copy = clone $this; $copy->sqlBuilder->notLike($lhs,$rhs); return $copy; }
	function notiLike($lhs, $rhs) { $copy = clone $this; $copy->sqlBuilder->notiLike($lhs,$rhs); return $copy; }
	...
	function contains($column, $point)	{ $copy = clone $this; $copy->sqlBuilder->contains($column, $point); return $copy; }
	//function distance($column, $distance) { $copy = clone $this; $copy->sqlBuilder->distance($column, $distance); return $copy; }
}
?>