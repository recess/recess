 ...
 */
class SelectPgSqlBuilderTest extends PHPUnit_Framework_TestCase  {
	
	protected $builder = null;
	
	function setUp() {
		$this->builder = new PgSqlBuilder();
	}
	...
	function testLeftOuterJoin() {
		...
		$expected = 'SELECT "books".* FROM "books" LEFT OUTER JOIN "authors" ON "authors"."id" = "books"."author_id" WHERE "authors"."first_name" = :_authors___first_name_';
		$this->assertEquals($expected, $this->builder->select());
	}
	
	function testInsert() {
		$this->builder->into('authors')->assign('first_name','Kris');
		$expected = 'INSERT INTO "authors" ("first_name") VALUES (:assgn_authors__first_name_)';
		$this->assertEquals($expected, $this->builder->insert());
	}
	
	function testUpdate() {
		$this->builder->table('authors')->equal('id',1)->assign('first_name','John');
		$expected = 'UPDATE "authors" SET "first_name" = :assgn_authors__first_name_ WHERE "authors"."id" = 1';
